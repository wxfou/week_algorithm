...
train_path = "train.in.txt"
test_path = "test.in.txt"
...
def test(A, B, PI, V, Q):
    print ("start testing")
    with open(test_path, "rb") as infile,open(output_path, "wb") as outfile:
        X_test = []
        y_test = []
        for line in infile:
            segs = line.decode().strip().split('\t')
            if len(segs) != 2: # 遇到空行时
                ...
                #print(str(X_test)+"\t"+str(y_test)+"\t"+str(preds)+ "\n")
                X_test = []
                y_test = []
            else:
                o = segs[0] # t时刻的观测o
